...
rulename=$merlinclash_uploadrulename
...
install_rule(){
	...
	cp -rf /tmp/kprule/$name/rules/daily.txt /koolshare/merlinclash/koolproxy/data/rules/
	cp -rf /tmp/kprule/$name/rules/koolproxy.txt /koolshare/merlinclash/koolproxy/data/rules
	cp -rf /tmp/kprule/$name/rules/kp.dat /koolshare/merlinclash/koolproxy/data/rules
	echo_date 规则更新完毕! >> $LOG_FILE
}
start_koolproxy(){
	echo_date "开启koolproxy进程... " >> $LOG_FILE

	/bin/sh /koolshare/scripts/clash_koolproxyconfig.sh restart
	cd
	rm -rf /tmp/kprule
}
