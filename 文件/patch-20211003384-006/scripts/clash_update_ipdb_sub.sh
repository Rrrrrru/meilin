...
update_ipdb(){
       if [ "$merlinclash_geoip_type" == "maxmind" ]; then
       echo_date "下载数据库来源为：$merlinclash_geoip_type" >> $LOG_FILE
       ...
       if [ "$merlinclash_geoip_type" == "ipip" ] ; then
              echo_date "下载数据库来源为：$merlinclash_geoip_type" >> $LOG_FILE
              if [ "x$wget" != "x" ] && [ -x $wget ]; then
       ...
       if [ "$merlinclash_geoip_type" == "Hackl0us" ] ; then
              echo_date "下载数据库来源为：$merlinclash_geoip_type" >> $LOG_FILE
              if [ "x$wget" != "x" ] && [ -x $wget ]; then
       ...
       if [ "$merlinclash_geoip_type" == "Loyalsoldier" ] ; then
              echo_date "下载数据库来源为：$merlinclash_geoip_type" 
              if [ "x$wget" != "x" ] && [ -x $wget ]; then
