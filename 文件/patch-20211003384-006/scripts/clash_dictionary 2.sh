...
if [ -f "$dictionary" ]; then #文件不存在则为首次订阅建立字典
	name_tmp=$(cat $dictionary | grep -w -n "$upname" | awk -F ":" '{print $1}')
		...
		if [ -n "$name_tmp" ]; then
			echo_date "同名配置存在，覆盖" >> $LOG_FILE
			sed -i "$name_tmp d" $dictionary
		else
			echo_date "配置不存在，新建" >> $LOG_FILE
		fi
		if [ "$subscription_type" == "1" ]; then #clash-yaml下载方式
			...
			echo \"name\":\"$upname\",\"link\":\"$merlinc_link\",\"type\":\"$subscription_type\",\"use\":\"0\",\"clashtarget\":\"$clashtarget\",\"acltype\":\"$acl4ssrsel\",\"emoji\":\"$emoji\",\"udp\":\"$udp\",\"appendtype\":\"$appendtype\",\"sort\":\"$sort\",\"fnd\":\"$fnd\",\"include\":\"$include\",\"exclude\":\"$exclude\",\"scv\":\"$scv\",\"tfo\":\"$tfo\",\"customrule\":\"0\",\"url\":\"$urlinilink\" >> $dictionary
		elif [ "$subscription_type" == "8" ]; then  #384_远程订阅 
			echo_date "【ACL4SSR远程订阅】" >> $LOG_FILE
			echo \"name\":\"$upname\",\"link\":\"$merlinc_link\",\"type\":\"$subscription_type\",\"use\":\"0\",\"clashtarget\":\"$clashtarget\",\"acltype\":\"$acl4ssrsel\",\"emoji\":\"$emoji\",\"udp\":\"$udp\",\"appendtype\":\"$appendtype\",\"sort\":\"$sort\",\"fnd\":\"$fnd\",\"include\":\"$include\",\"exclude\":\"$exclude\",\"scv\":\"$scv\",\"tfo\":\"$tfo\",\"addr\":\"$addr\",\"url\":\"$urlinilink\" >> $dictionary
		else
