...
move_list(){
	...
	if [ -f "/tmp/upload/$merlinclash_uploadlistfile" ]; then		
		cp -rf /tmp/upload/$merlinclash_uploadlistfile /koolshare/merlinclash/subconverter/rules/custom/$merlinclash_uploadlistfile
		rm -rf /tmp/upload/$merlinclash_uploadlistfile

		rm -rf /koolshare/merlinclash/yaml_bak/yamlscuslist.txt
