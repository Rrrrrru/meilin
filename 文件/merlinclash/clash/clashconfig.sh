...
set_Tolerance(){
	...
	if [ "$intervalbox" == "1" ]; then
		...
		#取interval值行数
		lines=$(cat $yamlpath | grep -n "\ \ \ \interval:" | awk -F ":" '{print $1}')
		for line in $lines; do
			sed -i "$line d" $yamlpath
			sed -i "$line i \ \ \ \ interval: $interval" $yamlpath
		done
	else
	...
	if [ "$urltestbox" == "1" ]; then
		...
		tlines=$(cat $yamlpath | grep -n "\ \ \ \tolerance:" | awk -F ":" '{print $1}')
		for line in $tlines; do
			sed -i "$line d" $yamlpath
			sed -i "$line i \ \ \ \ tolerance: $tolerance" $yamlpath
		done
	else
