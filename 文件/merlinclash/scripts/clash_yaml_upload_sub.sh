...
if [ -f "$yaml_tmp" ]; then
    echo_date "yaml文件【处理ing】，如【配置文件选择】下拉框未出现配置文件，请F5刷新页面！！！" >>"$LOG_FILE"
    sleep 2s
    #去注释
    ...
    echo_date "移动yaml文件到/koolshare/merlinclash/yaml_bak/" >> $LOG_FILE
    echo_date "以及/koolshare/merlinclash/yaml_use/目录下" >> $LOG_FILE
