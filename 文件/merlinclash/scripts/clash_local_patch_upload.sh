...
patchname=$(get merlinclash_uploadpatchname)
...
check_patchname(){
	...
	if [ -n "$patch_version" ]; then
		...
		mvl=$(get merlinclash_version_local)
		mcverson=$(echo $mvl | awk -F"." '{print $1}')
		#0724
		patchlocal=$(get merlinclash_patch_version)
		#0
		...
		if [ "$patchv1"x = "$mcverson"x ] && [ "$COMP" != "-1" ]; then
			...
			echo_date "当前插件版本号为：$mcverson" >> $LOG_FILE
			echo_date "准备安装补丁包对应插件版本号为：$patchv1" >> $LOG_FILE

			echo_date "当前已装补丁包版本号为：$patchlocal" >> $LOG_FILE
			echo_date "准备安装补丁包版本号为：$patchbag" >> $LOG_FILE
			echo_date "补丁安装须知：" >> $LOG_FILE
			echo_date "1.补丁包插件版本号需与插件版本号一致" >> $LOG_FILE
			echo_date "2.补丁包版本号需大于等于当前已装补丁版本号" >> $LOG_FILE
			cd
			clean
			echo BBABBBBC
			exit 1
...
replace_patch(){
	...
	if [ "$(pidof clash)" ];then
		echo_date "为了保证更新正确，关闭看门狗... " >> $LOG_FILE
