...
#flag为OTH且flag2为HND2，当前为AX56U，需要下载ARMV7_UPX二进制；flag为HND且flag2为HND2，为AX89X等
flag=$(get merlinclash_flag)
flag2=$(get merlinclash_flag2)
...
if [ "$flag2" == "HND2" ]; then
	url_main="https://raw.githubusercontent.com/flyhigherpi/merlinclash_clash_related/master/armv7_clash_binary_history" 
	url_cdn="https://cdn.jsdelivr.net/gh/flyhigherpi/merlinclash_clash_related/armv7_clash_binary_history"
fi
get_binary_history_cdn(){
    rm -rf /koolshare/merlinclash/clash_binary_history.txt
	rm -rf /tmp/upload/clash_binary_history.txt
	
    echo_date "从CDN地址下载clash历史版本号文件..." >> $LOG_FILE	
...
get_binary_history(){
	...
	if [ "$?" == "0" ];then
		echo_date "检查文件完整性" >> $LOG_FILE
       	if [ -z "$(cat /tmp/upload/clash_binary_history.txt)" ];then 
			echo_date "获取clash版本文件失败！使用CDN地址下载" >> $LOG_FILE
			get_binary_history_cdn
		fi
        #if [ -n "$(cat /tmp/upload/clash_binary_history.txt|grep "404")" ];then
		#	echo_date "error:404 | 获取clash版本文件失败！" >> $LOG_FILE
